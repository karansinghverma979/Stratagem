<script lang="ts">
  ...
  let {
    ...
    searchVal    = $bindable(''),
    isAscending  = $bindable(false),
    activeSort   = $bindable('By Resolution Date'),
    filters = [
      { id: 'ALL',  count: 0 },
      { id: 'HIGH', count: 0 },
      { id: 'MED',  count: 0 },
  ...
  const sortOptions = [
    { label: 'By Resolution Date', icon: 'calendar' },
    { label: 'By Creation Time',   icon: 'clock' },
    { label: 'By Name',            icon: 'az' },
    { label: 'By Priority',        icon: 'priority' }
  ];
  ...
  <div class="filter-bar">
    ...
    <div class="sort-container">
      <div class="dropdown-wrapper">
        <button class="sort-select-btn" id="archive-sort-toggle" onclick={() => showDropdown = !showDropdown}>
        ...
        {#if showDropdown}
          ...
          <div class="sort-dropdown" role="listbox" tabindex="-1" onmouseleave={() => showDropdown = false}>
            <div class="dropdown-header">ARCHIVE SORT PROTOCOL</div>
            {#each sortOptions as item}
              ...
              >
                <span class="di-icon">
                  ...
                  {:else if item.icon === 'priority'}
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M12 9V13M12 17H12.01M5.07183 19H18.9282C20.4678 19 21.4301 17.3333 20.6603 16L13.7321 4C12.9623 2.66667 11.0378 2.66667 10.2679 4L3.33975 16C2.56995 17.3333 3.53224 19 5.07183 19Z"/></svg>
                  {/if}
                </span>
...
<style>
  ...
  .tactical-btn.active {
    ...
    box-shadow: 0 0 28px rgba(139, 92, 246, 0.5), 0 0 10px rgba(139, 92, 246, 0.35), inset 0 1px 0 rgba(255,255,255,0.18);
    transform: translateY(-2px);
  ...
  .tactical-btn.active.tactical-btn-high {
    ...
    box-shadow: 0 0 32px rgba(255, 45, 85, 0.55), 0 0 12px rgba(255, 45, 85, 0.35), inset 0 1px 0 rgba(255,255,255,0.15);
  ...
  .tactical-btn.active.tactical-btn-med {
    ...
    box-shadow: 0 0 32px rgba(255, 184, 0, 0.5), 0 0 12px rgba(255, 184, 0, 0.32), inset 0 1px 0 rgba(255,255,255,0.15);
  ...
  .tactical-btn.active.tactical-btn-low {
    ...
    box-shadow: 0 0 32px rgba(0, 255, 159, 0.45), 0 0 12px rgba(0, 255, 159, 0.28), inset 0 1px 0 rgba(255,255,255,0.15);
  ...
  .tactical-btn.active.tactical-btn-high .active-bar { background: #ff2d55;             box-shadow: 0 0 8px rgba(255,45,85,0.8);  }
  .tactical-btn.active.tactical-btn-med .active-bar  { background: #ffb800;             box-shadow: 0 0 8px rgba(255,184,0,0.8);  }
  .tactical-btn.active.tactical-btn-low .active-bar  { background: #00ff9f;             box-shadow: 0 0 8px rgba(0,255,159,0.8);  }

  .btn-icon  { display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
  .btn-label { text-transform: uppercase; font-size: 15px; font-weight: 900; letter-spacing: 0.18em; }
  ...
  .tactical-btn-all.active .badge  { background: rgba(139,92,246,0.4); color: #d8b4fe; border-color: rgba(139,92,246,0.7); box-shadow: 0 0 10px rgba(139,92,246,0.5); text-shadow: 0 0 8px rgba(139,92,246,0.9); }
  .tactical-btn-high.active .badge { background: rgba(255,45,85,0.35);  color: #ffa0b4; border-color: rgba(255,45,85,0.7);  box-shadow: 0 0 10px rgba(255,45,85,0.55);  text-shadow: 0 0 8px rgba(255,45,85,0.9);  }
  .tactical-btn-med.active .badge  { background: rgba(255,184,0,0.3);   color: #fde68a; border-color: rgba(255,184,0,0.65); box-shadow: 0 0 10px rgba(255,184,0,0.5);   text-shadow: 0 0 8px rgba(255,184,0,0.9);   }
  .tactical-btn-low.active .badge  { background: rgba(0,255,159,0.25);  color: #6ee7b7; border-color: rgba(0,255,159,0.6);  box-shadow: 0 0 10px rgba(0,255,159,0.45);  text-shadow: 0 0 8px rgba(0,255,159,0.9);   }
  ...
  .search-container:focus-within .search-icon { color: rgba(139, 92, 246, 0.8); }
  ...
  .tactical-input:focus {
    ...
    box-shadow: inset 0 2px 12px rgba(0, 0, 0, 0.3), 0 0 24px rgba(139, 92, 246, 0.18);
  }
  ...
  .clear-btn:hover .clear-icon { transform: rotate(90deg); }
  ...
  .sort-icon-lead { color: rgba(139, 92, 246, 0.7); flex-shrink: 0; }
  .sort-prefix    { color: rgba(255, 255, 255, 0.55); font-size: 12px; font-weight: 900; letter-spacing: 0.2em; }
  .sort-value     { color: rgba(190, 155, 255, 1); text-shadow: 0 0 16px rgba(139, 92, 246, 0.7); font-size: 14px; font-weight: 900; max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

  .chevron-icon { color: rgba(255,255,255,0.4); transition: transform 0.3s ease; flex-shrink: 0; }
  ...
  .sort-dropdown {
    ...
    right: 0;
    width: 280px;
    background: linear-gradient(145deg, rgba(20,12,45,0.97) 0%, rgba(10,8,25,0.97) 100%);
    backdrop-filter: blur(40px) saturate(200%);
    -webkit-backdrop-filter: blur(40px) saturate(200%);
    border: 1px solid rgba(139, 92, 246, 0.35);
    border-radius: 14px;
    box-shadow: 0 24px 60px rgba(0,0,0,0.8), 0 0 40px rgba(109,40,217,0.12), inset 0 1px 0 rgba(255,255,255,0.06);
    display: flex;
  ...
  .dropdown-item:hover         { background: rgba(139,92,246,0.12); color: #fff; padding-left: 18px; }
  .dropdown-item.selected      { background: rgba(139,92,246,0.18); color: #fff; border: 1px solid rgba(139,92,246,0.3); }

  .di-icon { display: flex; align-items: center; flex-shrink: 0; color: rgba(139,92,246,0.65); transition: color 0.2s ease; }
  .dropdown-item:hover .di-icon, .dropdown-item.selected .di-icon { color: rgba(139,92,246,1); }
  .di-label { flex: 1; }
  .di-check { display: flex; align-items: center; color: rgba(139,92,246,0.9); margin-left: auto; }
  ...
  .sort-dir-icon { transition: transform 0.4s cubic-bezier(0.16, 1, 0.3, 1); flex-shrink: 0; }
  .sort-dir-icon.descending { transform: scaleY(-1); }

  .dir-label { font-size: 12px; font-weight: 900; letter-spacing: 0.14em; min-width: 34px; text-align: left; }
</style>
