<script lang="ts">
  import { archivedTasks } from '../../../core/store';
  ...
  let allUniqueTags = $derived.by(() => {
    ...
    $archivedTasks.forEach(task => {
      ...
      if (task.classifications && Array.isArray(task.classifications)) {
        task.classifications.forEach((tag: string) => {
          const cleaned = tag.trim().toUpperCase().replace('#', '');
          if (cleaned) tagsSet.add(cleaned);
        });
      }
...
<div class="tag-bar-wrapper">
  <div class="tag-bar-container">
    <button class="scroll-btn scroll-left" onclick={() => scroll('left')} aria-label="Scroll Left">
    ...
    <button class="scroll-btn scroll-right" onclick={() => scroll('right')} aria-label="Scroll Right">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
<style>
  ...
  .tag-pill-wrapper.active .tag-pill   { color: #ffffff !important; text-shadow: 0 0 10px #ffffff; }
  .tag-pill-wrapper.active .favorite-btn { border-left-color: rgba(255, 255, 255, 0.2); }
  ...
  .favorite-btn:hover { color: rgba(255, 255, 255, 0.8); background: rgba(255, 255, 255, 0.08); }
  ...
  .favorite-btn svg { width: 14px; height: 14px; transition: transform 0.3s var(--ease-fui); }
  .favorite-btn:hover svg { transform: scale(1.2) rotate(15deg); }
</style>
