<script lang="ts">
  ...
  let allUniqueTags = $derived.by(() => {
    ...
    $arsenalTasks.forEach(task => {
      if (task.tags && Array.isArray(task.tags)) {
        task.tags.forEach(tag => {
          const cleaned = tag.trim().toUpperCase().replace('#', '');
          // Exclude internal tags
          if (cleaned && cleaned !== 'RAW' && cleaned !== 'SYNTH' && cleaned !== 'WEAPON' && cleaned !== 'HIGH' && cleaned !== 'MED' && cleaned !== 'LOW' && cleaned !== 'RAW_INTEL' && cleaned !== 'SYNTHESIZING') {
            tagsSet.add(cleaned);
          }
        });
      }
...
<style>
  ...
  :global(.tag-raw-intel.active) {
    background: rgba(139, 92, 246, 0.35) !important;
    border-color: var(--primary-accent) !important;
    color: #ffffff !important;
    box-shadow: 0 0 25px var(--primary-glow), inset 0 0 8px rgba(139, 92, 246, 0.4);
    animation: selectionPulse 0.4s cubic-bezier(0.16, 1, 0.3, 1);
  }

  :global(.tag-synthesizing.active) {
    background: rgba(6, 182, 212, 0.3) !important;
    border-color: #06b6d4 !important;
    color: #ffffff !important;
    box-shadow: 0 0 25px rgba(6, 182, 212, 0.45), inset 0 0 8px rgba(6, 182, 212, 0.3);
    animation: selectionPulse 0.4s cubic-bezier(0.16, 1, 0.3, 1);
  }
