<script lang="ts">
  ...
  import AbortConfirmation from '../../Execution/ExecutionTaskView/AbortConfirmation.svelte';
  import StrategizeModal from '../../Arsenal/ArsenalMain/StrategizeModal.svelte';
  
  let { isOpen = false, mission = null, onclose, onreschedule } = $props();
  let readOnly = $derived(mission && (mission.resolution || mission.status === 'VICTORY' || mission.status === 'ABORTED'));

  let overdueDays = $derived.by(() => {
    if (!mission || !mission.deadlineDate) return 0;
    const deadlineTime = new Date(mission.deadlineDate);
  ...
  let rescheduleCount = $derived(mission?.rescheduleCount || (mission?.isRescheduled ? 1 : 0));
  ...
  const handleRescheduleClick = () => {
    if (!isReschedulable) {
      ...
      triggerShield(lockReason, 'CRITICAL', 4000);
    } else {
      AudioEngine.play('success');
      onreschedule && onreschedule(mission);
    }
  };
  ...
  let isAbortConfirmOpen = $state(false);// 3D perspective mouse tilt tracking (Forge alignment)
  let tiltX = $state(0);
  let tiltY = $state(0);
  ...
  $effect(() => {
    if (isOpen && mission) {
      ...
      deadline = mission.deadlineDate || mission.completionDate || '';
  ...
  const updateThreatLevelInDB = async (level: string) => {
    ...
    try {
      if (window.stratagemAPI && window.stratagemAPI.updateMissionThreatLevel) {
        await window.stratagemAPI.updateMissionThreatLevel(mission.id, level);
        await syncAntaryami();
        await loadAuditLogs();
      }
    } catch (e) {
...
{#if isOpen}
  ...
  <div class="overlay-backdrop" onclick={triggerClose} role="presentation" transition:fade={{ duration: 150 }}>
    ...
    <!-- Modal panel (Themed like the Forge with 3D Tilt) -->
    <div 
    ...
    >
            ...
            >
              ...
              <div class="scope-reticle-core">
                ...
                <svg class="reticle-svg" width="100" height="100" viewBox="0 0 100 100" fill="none">
                  ...
                  <defs>
                    <linearGradient id="breach-primaryGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" stop-color="var(--primary-accent)" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" />
                    </linearGradient>
                    <linearGradient id="breach-utilityGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" stop-color="var(--tertiary-accent)" />
                      <stop offset="100%" stop-color="var(--secure-status)" />
                    </linearGradient>
                    <linearGradient id="breach-warningGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" stop-color="var(--warning-amber)" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" />
                    </linearGradient>
                    <linearGradient id="breach-alertGrad" x1="0%" y1="0%" x2="100%" y2="100%">
                      <stop offset="0%" stop-color="var(--critical-alert)" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" />
                    </linearGradient>

                    <linearGradient id="breach-spotlightNorthGrad" x1="0%" y1="100%" x2="0%" y2="0%">
                    ...
                    <linearGradient id="breach-spotlightSouthGrad" x1="0%" y1="0%" x2="0%" y2="100%">
                      <stop offset="0%" stop-color="var(--tertiary-accent)" stop-opacity="1" />
                      <stop offset="35%" stop-color="var(--primary-accent)" stop-opacity="0.8" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" stop-opacity="0" />
                    </linearGradient>
                    <linearGradient id="breach-spotlightWestGrad" x1="100%" y1="0%" x2="0%" y2="0%">
                      <stop offset="0%" stop-color="var(--tertiary-accent)" stop-opacity="1" />
                      <stop offset="35%" stop-color="var(--primary-accent)" stop-opacity="0.8" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" stop-opacity="0" />
                    </linearGradient>
                    <linearGradient id="breach-spotlightEastGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                      <stop offset="0%" stop-color="var(--tertiary-accent)" stop-opacity="1" />
                      <stop offset="35%" stop-color="var(--primary-accent)" stop-opacity="0.8" />
                      <stop offset="100%" stop-color="var(--secondary-accent)" stop-opacity="0" />
                    </linearGradient>

                    <linearGradient id="breach-brightCrossGrad" x1="-286" y1="50" x2="386" y2="50" gradientUnits="userSpaceOnUse">
                  ...
                  <g class="massive-plus-crosshair">
                    <line x1="-286" y1="50" x2="386" y2="50" stroke="url(#breach-utilityGrad)" stroke-width="0.75" class="plus-line plus-horizontal" />
                    <line x1="-286" y1="43" x2="-286" y2="57" stroke="url(#breach-utilityGrad)" stroke-width="1.5" class="plus-tick tick-left" />
                    <line x1="386" y1="43" x2="386" y2="57" stroke="url(#breach-utilityGrad)" stroke-width="1.5" class="plus-tick tick-right" />

                    <line x1="50" y1="-286" x2="50" y2="386" stroke="url(#breach-utilityGrad)" stroke-width="0.75" class="plus-line plus-vertical" />
                    <line x1="43" y1="-286" x2="57" y2="-286" stroke="url(#breach-utilityGrad)" stroke-width="1.5" class="plus-tick tick-top" />
                    <line x1="43" y1="386" x2="57" y2="386" stroke="url(#breach-utilityGrad)" stroke-width="1.5" class="plus-tick tick-bottom" />
                  </g>

                  <!-- Spotlight beams -->
                  <g class="spotlight-breath-wrapper">
                    <line x1="50" y1="50" x2="50" y2="-454" stroke="url(#breach-spotlightNorthGrad)" class="spotlight-beam beam-north" />
                    <line x1="50" y1="50" x2="50" y2="386" stroke="url(#breach-spotlightSouthGrad)" class="spotlight-beam beam-south" />
                    <line x1="50" y1="50" x2="-454" y2="50" stroke="url(#breach-spotlightWestGrad)" class="spotlight-beam beam-west" />
                    <line x1="50" y1="50" x2="386" y2="50" stroke="url(#breach-spotlightEastGrad)" class="spotlight-beam beam-east" />
                  </g>

                  <!-- Rotating perpendicular crossing lines -->
                  <g class="rotating-crossing-lines-group">
                    <line x1="-286" y1="50" x2="386" y2="50" stroke="url(#breach-brightCrossGrad)" stroke-width="0.5" class="rotating-cross-line" />
                    <line x1="50" y1="-286" x2="50" y2="386" stroke="url(#breach-brightCrossGrad)" stroke-width="0.5" class="rotating-cross-line" />
                  </g>

                  <!-- Concentric broken rings -->
                  <g class="outer-ring-breath-wrapper">
                    <circle cx="50" cy="50" r="38" stroke="url(#breach-primaryGrad)" stroke-width="4.5" stroke-dasharray="24 10" class="outer-broken-ring" />
                  </g>

                  <g class="inner-ring-breath-wrapper">
                    <circle cx="50" cy="50" r="23" stroke="url(#breach-utilityGrad)" stroke-width="3.5" stroke-dasharray="12 6" class="inner-broken-ring" />
                  </g>

                  <!-- Sliding mechanical crosshairs -->
                  <line x1="50" y1="4" x2="50" y2="18" stroke="url(#breach-primaryGrad)" class="crosshair-line line-top" />
                  <line x1="50" y1="96" x2="50" y2="82" stroke="url(#breach-primaryGrad)" class="crosshair-line line-bottom" />
                  <line x1="4" y1="50" x2="18" y2="50" stroke="url(#breach-primaryGrad)" class="crosshair-line line-left" />
                  <line x1="96" y1="50" x2="82" y2="50" stroke="url(#breach-primaryGrad)" class="crosshair-line line-right" />

                  <!-- Dynamic center threat icon -->
                  {#if threatLevel === 'LOW'}
                    <path d="M 45,44 L 50,41 L 55,44 L 55,49 C 55,53 50,56 50,56 C 50,56 45,53 45,49 Z" stroke="url(#breach-utilityGrad)" stroke-width="3" fill="rgba(0, 255, 159, 0.12)" class="center-lock-icon" />
                  {:else if threatLevel === 'MED'}
                    <path d="M 50,41 L 59,50 L 50,59 L 41,50 Z" stroke="url(#breach-warningGrad)" stroke-width="3" fill="rgba(255, 184, 0, 0.12)" class="center-lock-icon" />
                  {:else}
                    <g class="center-lock-icon">
                      <path d="M 44,44 L 56,56 M 56,44 L 44,56" stroke="url(#breach-alertGrad)" stroke-width="3" />
                      <circle cx="50" cy="50" r="5" fill="rgba(255, 45, 85, 0.2)" stroke="url(#breach-alertGrad)" stroke-width="3" class="center-danger-lock-circle" />
                    </g>
                  {/if}
      ...
      <div class="body-grid">
        ...
        <section class="grid-column left-info">
          <div class="left-info-card" onclick={registerClick} role="presentation">
            ...
            <div class="left-info-scroll-container custom-scroll">
              ...
              <div class="info-group">
                <div class="info-group-header">
                  <svg class="elem-icon days-icon" class:days-icon-overdue={calculateDaysLeft(deadline).includes('OVERDUE')} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <span class="info-label font-outfit">DAYS REMAINING</span>
                </div>
                <div class="read-only-pill days-pill-cyber font-outfit" class:overdue-pill-cyber={calculateDaysLeft(deadline).includes('OVERDUE')}>
        ...
        <section class="grid-column center-subtask">
          ...
          <div class="center-subtask-container">
            ...
            {#if !readOnly}
              <div class="add-subtask-row-cyber">
                <button class="add-btn-cyber" onclick={addObjective} aria-label="Add subtask">
                ...
                <input 
                  ...
                  placeholder="DEPLOY NEW TARGET NODE..." 
      ...
      <footer class="modal-footer">
        ...
        <div class="footer-right">
          {#if readOnly}
            <button class="action-btn-cyber close-archive-btn-cyber font-outfit" onclick={triggerClose}>
              DISCONNECT UPLINK
            </button>
          {:else}
            ...
            <button class="action-btn-cyber reschedule-btn-cyber font-outfit" class:disabled={!isReschedulable} onclick={handleRescheduleClick}>
              <svg class="btn-icon sync-icon" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
...
<style>
  /* Scoped color tokens fallback for Breach Red/Rose Theme */
  .overlay-backdrop {
    --primary-accent: #ff2d55;
    --secondary-accent: #fda4af;
    --tertiary-accent: #ffb800;
    --secure-status: #10b981;
    --critical-alert: #ff2d55;
    --primary-glow: rgba(255, 45, 85, 0.4);
    --secondary-glow: rgba(253, 164, 175, 0.25);
    --tertiary-glow: rgba(255, 184, 0, 0.3);
  }
  ...
  .ring-cyan {
    ...
    border-color: var(--tertiary-accent); border-style: dashed;
    animation: data-ring-breath-a 6s ease-in-out infinite alternate;
  }
  .ring-violet {
    top: 8px; left: 8px; right: 8px; bottom: 8px;
    border-color: var(--primary-accent); border-style: dotted;
  ...
  .acquired .plus-line {
    opacity: 0.85;
    stroke: url(#breach-alertGrad) !important;
    stroke-width: 1.2px;
  }
  ...
  .forge-title {
    ...
    margin: 0;
    letter-spacing: 3px;
    background: linear-gradient(90deg, #fff, var(--primary-accent), var(--tertiary-accent), #fff);
  ...
  .right-audit-scroll-container::-webkit-scrollbar-thumb {
    background: linear-gradient(180deg, var(--primary-accent) 0%, var(--tertiary-accent) 100%);
    border-radius: 4px;
  ...
  .tile-title-cyber {
    ...
    background: linear-gradient(90deg, var(--secondary-accent) 0%, #a78bfa 50%, var(--tertiary-accent) 100%);
    background-size: 200% auto;
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
  ...
  .init-pill-cyber {
    background: linear-gradient(135deg, var(--primary-accent) 0%, rgba(255, 45, 85, 0.8) 50%, rgba(100, 10, 20, 0.9) 100%);
  ...
  .tag-preset-1 {
    background: linear-gradient(135deg, var(--primary-accent) 0%, rgba(200, 20, 40, 0.9) 50%, rgba(120, 12, 24, 0.95) 100%);
  ...
  .add-btn-cyber:hover {
    ...
    filter: drop-shadow(0 0 6px var(--tertiary-accent));
  }
  ...
  .flow-desc {
    font-size: 1.15rem; /* Chronological telemetry scaled up to 1.15rem */
    font-weight: 500;
    line-height: 1.4;
    color: rgba(255, 255, 255, 0.85);
  }
  ...
  .progress-bar-fill-cyber {
    height: 100%;
    background: linear-gradient(90deg, #ec4899 0%, var(--primary-accent) 50%, var(--tertiary-accent) 100%);
    border-radius: 9px;
    position: relative;
  ...
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-ring) {
    animation: victoryRingSpin 2s linear infinite;
    opacity: 0.95;
    color: #ffffff;
    filter: drop-shadow(0 0 3px var(--tertiary-accent));
  }
  ...
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-cup),
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-handle-l),
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-handle-r),
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-base) {
  ...
  :global(.victory-btn-cyber:hover:not(:disabled) .victory-icon-sparkle) {
    opacity: 0.85;
  ...
  .victory-btn-cyber {
    background: linear-gradient(135deg, var(--tertiary-accent) 0%, #0891b2 50%, #0c4a6e 100%);
  ...
  .close-archive-btn-cyber {
    background: linear-gradient(135deg, var(--primary-accent) 0%, rgba(255, 45, 85, 0.8) 100%);
    border: 2px solid rgba(139, 92, 246, 0.6);
    color: #ffffff;
    border-radius: 34px;
  ...
  /* Breach Reschedule Button Styles */
  .reschedule-btn-cyber {
    background: linear-gradient(135deg, #06b6d4 0%, #0891b2 50%, #0c4a6e 100%) !important;
    border: 2px solid rgba(6, 182, 212, 0.65) !important;
  ...
  .reschedule-btn-cyber:hover:not(.disabled) {
  ...
  .reschedule-btn-cyber.disabled {
    background: linear-gradient(135deg, rgba(8, 20, 24, 0.8) 0%, rgba(6, 14, 18, 0.9) 100%) !important;
    border: 2px solid rgba(255, 45, 85, 0.35) !important;
  ...
  .reschedule-btn-cyber.disabled:hover {
    border-color: rgba(255, 45, 85, 0.8) !important;
    color: var(--critical-alert) !important;
  ...
  .reschedule-btn-cyber:hover:not(.disabled) .sync-icon {
